...
class Proxy {
  ...
  async update(opt) {
    ...
    try{
      ...
      const old = opt.old;
      const query = 'UPDATE ' + process.env.PROXY_TABLE + ' SET PROXYID=\'' + newJson.proxy + '\', TARGETID=\'' + newJson.target + '\', PROXYLEVEL=\'' + newJson.level + '\' WHERE PROXYID=\'' + old.proxy + '\' AND TARGETID=\'' + old.target + '\' AND PROXYLEVEL=\'' + old.level + '\'';
      let result = await connection.execute(query, [],{ autoCommit: true });
      console.log(result);
      if(connection){
        try{
